    ...
    <table class="table table-hover table-consended">
        ...
        <tbody>
            <?php foreach ($data as $name => $item): ?>
                <tr>
                    <td><a href='#' class="custom-count-incircle" data-href='<?= Yii::$app->homeUrl ?>user/poidetail-incircle?lat=<?= $params['lat'] ?>&lng=<?= $params['lng'] ?>&radius=<?= $params['radius'] ?>&poi=<?= $name ?>' data-toggle='collapse' data-target="#<?= $name ?>"><?= $item['title'] ?></a></td>
